...
- `morse_gemini` — `morse_keyer` の拡張版。prosign(`KA`/`AR`/`K`/`SK`)でハンドオーバーすると解読テキストを Google Gemini API に HTTPS POST し、AI の返信をモールス(D6 LED + D5 ブザー)で送出するリアル AI 交信装置。Gemini は「ハム無線オペレーター」として Q-コード/略語のみで返答するよう指示済み。WiFi 失敗時はローカル打鍵モードで動作継続、API エラー時は `QRX`(接続系)/`AGN`(応答系)を送出してデグレード。`SECRET_GEMINI_APIKEY`(Google AI Studio で発行)+ `SECRET_SSID`/`SECRET_PASS` + WiFi 必須。lib: `LiquidCrystal_I2C`、`ArduinoJson`。I2C 外部 pull-up 必須。配線は `morse_keyer` と同一(Wokwi diagram.json あり)。
